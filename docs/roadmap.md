...
| 模块 | 能力 | 实现思路 |
|------|------|----------|
| **伏笔健康** | 开放伏笔列表 + 「埋设章 → 当前章」间隔 + 超期标红 | 已有 `foreshadows` 表，加 UI + 规则（如 >20 章未收警告） |
| **实体冲突** | 同一实体 `state_json` 前后矛盾检测 | Extract 后 diff 对比；或扩展 `nova doctor` |
| **记忆冲突** | 已有 conflict 检测 → 加 **确认/合并/归档** 工作流 | MemoryPanel 从「只报告」升级为 actionable |
| **跨章矛盾** | 轻量版：Extract 新事实 vs 已有 entities/memories 冲突 | 可先规则 + LLM 二次确认 |

**入口**：Studio 新 Tab「一致性」或 Overview 扩展区
...
- 冲突处理闭环：发现 → 作者确认 → 归档/修正 memory 或 entity
...
| 能力 | 说明 |
|------|------|
| 步骤可视化 | WriteStepper 已有，补齐每步耗时、可跳过审查（高级选项） |
| 上下文预览 | WriteContextPanel 展示 A1 召回结果，写前可手动 pin/排除记忆 |
| 断点续写 UI | `--resume` 在 Desktop 一键「继续上次未完成流水线」 |
...
1. 「基于已写内容重新规划第 N 卷」工作流
2. Plan Agent 输入：总纲 + 已写摘要链 + entities + 开放伏笔
3. 输出：新卷纲 diff，作者确认后替换
4. 可选：标记哪些原章纲条目已「偏离/完成/废弃」
...
| 功能 | 说明 |
|------|------|
| 更新目标 | 每日字数/章数目标（项目级配置） |
| 存稿计数 | 「已写未发布」章数（手动标记 published 或 scheduled） |
| Streak / 日历 | 简单本地记录，无需云端 |
| Overview 卡片 | 「今日建议：写第 N 章 + 审查第 M 章」 |
